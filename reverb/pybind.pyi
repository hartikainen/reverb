...
class Sampler:
  ...
  def GetNextTimestepBatch(self, batch_size: int, timeout_as_end: bool = False) -> List[np.ndarray]: ...

  def GetNextTrajectoryBatch(self, batch_size: int, timeout_as_end: bool = False) -> List[np.ndarray]:
    ...
...
class Client:
  ...
  def NewSampler(self,
      ...
      rate_limiter_timeout_ms: int = -1, max_samples_per_stream: int = -1) -> Sampler:
    ...
...
class PatternWriter:
  def __init__(self, configs: List[bytes], history: int): ...
  def Append(self, data: List[np.ndarray], end_episode: bool,
             clear_buffers: bool) -> List[List[np.ndarray]]: ...
  def SetBlock(self, data: List[np.ndarray], ends: np.ndarray) -> None: ...
  def Read(self, batch_size: int, clear_buffers: bool,
           flush: bool) -> List[np.ndarray]: ...
  def Close(self) -> None: ...
